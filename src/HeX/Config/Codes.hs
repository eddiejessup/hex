...
instance Enum DelimiterCode where
    toEnum n
        ...
        | otherwise = let smallVar = toEnum $ n `shiftR` 12
                          largeVar = toEnum $ n .&. 0xFFF
                      in
                          DelimiterSpecCode $ DelimiterSpec smallVar largeVar
...
instance Enum FamilyCharRef where
    ...
    fromEnum (FamilyCharRef fam pos) = (fam `shiftL` 8) + pos
...
instance Enum MathCode where
    toEnum n
        ...
        | otherwise = let cls = toEnum $ n `shiftR` 12
                          fam = toEnum $ n .&. 0xFFF
                      in
                          NormalMathCode cls fam

    fromEnum ActiveMathCode = 0x8000
    fromEnum (NormalMathCode cls famRef) = fromEnum famRef
        + (fromEnum cls `shiftL` 12)
...
newMathCodes = initialiseCharCodes f
  where
    f c
        | c `elem` digits =
            NormalMathCode VariableFamily (FamilyCharRef 0 $ ord c)
        | c `elem` letters =
            NormalMathCode VariableFamily (FamilyCharRef 1 $ ord c)
...
data CaseChangeCode = NoCaseChange | ChangeToCode Cat.CharCode
    deriving ( Show )
