...
instance Show Glue where
  ...
  show (Glue d str shr) =
    "{" ++ Unit.showSP d ++ ("+" ++ show str) ++ ("-" ++ show shr) ++ "}"
...
instance Show BreakableHListElem where
  ...
  showList a = (show (foldr append [] a) ++)
    where
      ...
      append y r@(x:xs)
        | HCharacter B.Character {code = n} <- y
...
addGlueFlex g1@GlueFlex {factor = f1, order = o1} g2@GlueFlex { factor = f2
                                                              , order = o2
                                                              } =
...
flex = listFlex . mapMaybe toGlue
  where
    ...
    listFlex gs =
      let (strs, shrs) =
            unzip [(str, shr) | Glue {stretch = str, shrink = shr} <- gs]
...
listStatusBadness NaturallyGood = FiniteBadness 0
listStatusBadness (NaturallyBad Full Unfixable) = InfiniteBadness
listStatusBadness (NaturallyBad Bare Unfixable) = FiniteBadness tenK
...
listStatusBadness (NaturallyBad _ Fixable {order = _}) = FiniteBadness 0
