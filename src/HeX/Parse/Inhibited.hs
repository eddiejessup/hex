...
unsafeParseMacroArgs MacroContents {preParamTokens=pre, parameters=params} = do
  ...
  parseArgs params
  where
    parseArgs :: (P.Stream s, P.Token s ~ PrimitiveToken) => Map.Map Digit [Lex.Token] -> SimpParser s (Map.Map Digit MacroArgument)
    ...
    -- following {...} group.
    parseUndelimitedArgs :: (P.Stream s, P.Token s ~ PrimitiveToken) => SimpParser s [Lex.Token]
    ...
    parseDelimitedArgs :: (P.Stream s, P.Token s ~ PrimitiveToken) => [Lex.Token] -> [Lex.Token] -> SimpParser s [Lex.Token]
...
maybeParseParametersFrom :: (P.Stream s, P.Token s ~ PrimitiveToken) => Digit -> SimpParser s MacroParameters
...
unsafeParseParamText :: (P.Stream s, P.Token s ~ PrimitiveToken) => SimpParser s ([Lex.Token], MacroParameters)
...
unsafeParseBalancedText :: (P.Stream s, P.Token s ~ PrimitiveToken) => TerminusPolicy -> SimpParser s BalancedText
