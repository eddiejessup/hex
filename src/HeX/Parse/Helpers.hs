...
freshPosState stream =
    P.PosState { P.pstateInput      = stream
               , P.pstateOffset     = 0
...
freshState stream =
    P.State { P.stateInput    = stream
            , P.stateOffset   = 0
            , P.statePosState = freshPosState stream
            }

easyRunParser :: SimpParser s a
              -> s
              -> (P.State s, Either (ParseErrorBundle s) a)
...
copyState _state tgtStream =
    ...
    in
        P.State { P.stateInput    = tgtStream
                , P.stateOffset   = offset
  ...
  where
    copyPosState posState =
        ...
        in
            P.PosState { P.pstateInput      = tgtStream
                       , P.pstateOffset     = offset
                       , P.pstateSourcePos  = sourcePos
                       , P.pstateTabWidth   = tabWidth
                       , P.pstateLinePrefix = linePrefix
