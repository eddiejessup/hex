...
chopDropWhile getNext test cs =
  case getNext cs of
    ...
    Just (next, rest) ->
      if test next
        then chopDropWhile getNext test rest
        else rest
...
extractToken getCC state cs = do
  ...
  where
    extractTokenRest (Cat.CharCat n cat1) rest
      ...
      , Cat.Space <- cat1 = extractToken getCC state rest
      | SkippingBlanks <- state
      , Cat.EndOfLine <- cat1 = extractToken getCC state rest
      -- Space in middle of line: Make a space token and start skipping blanks.
      | LineMiddle <- state
      , Cat.Space <- cat1 = Just (spaceTok, SkippingBlanks, rest)
      -- End of line in middle of line: Make a space token and go to line begin.
      | LineMiddle <- state
      , Cat.EndOfLine <- cat1 = Just (spaceTok, LineBegin, rest)
      -- Ignored: Ignore.
