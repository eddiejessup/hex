...
getSelectFontNrInstruction fNr
    | fNr < 64 = pure $
        getSimpleEncInstruction $ SelectFontNr $ FastSelectFontOp $ fromIntegral fNr
    | otherwise =
...
getPostPostambleInstr postamblePointer =
  ...
  where
    args = [ (SIntArgVal . S4 . fromIntegral) postamblePointer, dviFormatArg ]
        <> signature
